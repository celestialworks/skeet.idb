...
bool c_lag_comp::StartPrediction(AimPlayer* data) {
	...
	if (std::abs(c_client::get()->m_arrival_tick - simulation) >= 128)
		return true;
	...
	int updatedelta = c_client::get()->m_server_tick - record->m_tick;
	...
	if (c_client::get()->m_latency_ticks <= lag - updatedelta)
		return true;

	// the next update will come in, wait for it.
	int next = record->m_tick + 1;
	if (next + lag >= c_client::get()->m_arrival_tick)
		return true;
	...
	if (size > 1) {
		...
		change = (math::NormalizedAngle(dir - prevdir) / dt) * c_csgo::get()->m_globals->m_interval;
	}
	...
	while (true) {
		...
		next += lag;
		if (next >= c_client::get()->m_arrival_tick)
			break;
		...
		for (int sim{}; sim < lag; ++sim) {
			...
			if (record->m_pred_flags & FL_ONGROUND) {
				if (!c_csgo::get()->sv_enablebunnyhopping->GetInt()) {
				...
				record->m_pred_velocity.z = c_csgo::get()->sv_jump_impulse->GetFloat();
			}
			...
			// apply gravity and airaccel.
				...
				record->m_pred_velocity.z -= c_csgo::get()->sv_gravity->GetFloat() * c_csgo::get()->m_globals->m_interval;
			...
			record->m_pred_time += c_csgo::get()->m_globals->m_interval;
	...
	c_bones::get()->setup(data->m_player, nullptr, record);

	return true;
}

void c_lag_comp::PlayerMove(LagRecord* record) {
	...
	end = start + (record->m_pred_velocity * c_csgo::get()->m_globals->m_interval);

	// trace.
	c_csgo::get()->m_engine_trace->TraceRay(Ray(start, end, record->m_mins, record->m_maxs), CONTENTS_SOLID, &filter, &trace);
	...
	if (trace.m_fraction != 1.f) {
		...
		for (int i{}; i < 2; ++i) {
			...
			end = start + (record->m_pred_velocity * (c_csgo::get()->m_globals->m_interval * (1.f - trace.m_fraction)));
...
void c_lag_comp::AirAccelerate(LagRecord* record, ang_t angle, float fmove, float smove) {
	...
	accelspeed = c_csgo::get()->sv_airaccelerate->GetFloat() * wishspeed * c_csgo::get()->m_globals->m_interval;
...
void c_lag_comp::PredictAnimations(CCSGOPlayerAnimState* state, LagRecord* record) {
	struct AnimBackup_t {
		float  curtime;
		float  frametime;
		int    flags;
	...
	// backup data.
	AnimBackup_t backup;
	backup.curtime = c_csgo::get()->m_globals->m_curtime;
	backup.frametime = c_csgo::get()->m_globals->m_frametime;
	...
	c_csgo::get()->m_globals->m_curtime = record->m_pred_time;
	...
	bool fake = c_config::get()->b["rbot_resolver"];

	// rerun the resolver since we edited the origin.
	if (fake)
		c_resolver::get()->ResolveAngles(player, record);
	...
	// restore globals.
	c_csgo::get()->m_globals->m_curtime = backup.curtime;
	c_csgo::get()->m_globals->m_frametime = backup.frametime;
