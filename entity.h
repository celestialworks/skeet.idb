...
public:
	__forceinline Player *GetPlayer() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle< Player * >(m_hPlayer());
	}
...
public:
	...
	__forceinline bool m_bIsLocalPlayer() {
		...
		//return get< bool >(c_csgo::get()->IsLocalPlayer);
		return  c_csgo::get()->m_engine->GetLocalPlayer() == this->index();
	}
	...
	__forceinline C_AnimationLayer *m_AnimOverlay() {
		...
		// .text:1017EAC6
		return get< C_AnimationLayer * >(c_csgo::get()->AnimOverlay);
	}
	...
	__forceinline CBoneAccessor &m_BoneAccessor() {
		...
		return get< CBoneAccessor >(c_csgo::get()->BoneAccessor);
	}
...
public:
	...
	__forceinline CStudioHdr *GetModelPtr() {
		...
		if (!m_studioHdr())
			c_csgo::get()->LockStudioHdr.as< LockStudioHdr_t >()(this);

		return m_studioHdr();
	}

	__forceinline Weapon *GetActiveWeapon() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle< Weapon * >(m_hActiveWeapon());
	}

	__forceinline Entity *GetObserverTarget() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle(m_hObserverTarget());
	}

	__forceinline Entity *GetGroundEntity() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle(m_hGroundEntity());
	}
	...
	__forceinline bool ComputeHitboxSurroundingBox(vec3_t *mins, vec3_t *maxs) {
		...
		return c_csgo::get()->ComputeHitboxSurroundingBox.as< ComputeHitboxSurroundingBox_t >()(this, mins, maxs);
	}

	__forceinline int GetSequenceActivity(int sequence) {
		using GetSequenceActivity_t = int(__fastcall *)( void*, CStudioHdr *, int );

		return c_csgo::get()->GetSequenceActivity.as< GetSequenceActivity_t >()( this, GetModelPtr(), sequence );
	}

	__forceinline bool HasC4() {
		using HasC4_t = bool(__thiscall *)(decltype(this));
		return c_csgo::get()->HasC4.as< HasC4_t >()(this);
	}
	...
	__forceinline bool enemy(Player *from) {
		...
		else if (c_csgo::get()->mp_teammates_are_enemies->GetInt())
			return true;
...
public:
	...
	__forceinline Weapon *GetWeapon() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle< Weapon * >(m_hWeapon());
	}

	__forceinline Weapon *GetWeaponWorldModel() {
		return c_csgo::get()->m_entlist->GetClientEntityFromHandle< Weapon * >(m_hWeaponWorldModel());
	}
	...
	__forceinline vec3_t CalculateSpread(int seed, float inaccuracy, float spread, bool revolver2 = false) {
		...
		// seed randomseed.
		c_csgo::get()->RandomSeed((seed & 0xff) + 1);

		// generate needed floats.
		r1 = c_csgo::get()->RandomFloat(0.f, 1.f);
		r2 = c_csgo::get()->RandomFloat(0.f, math::twopi);
		...
		*/
		...
		if ( /*wep_info->m_weapon_type == WEAPONTYPE_SHOTGUN &&*/ c_csgo::get()->weapon_accuracy_shotgun_spread_patterns->GetInt() > 0)
			c_csgo::get()->GetShotgunSpread(item_def_index, 0, 0 /*bullet_i*/ + wep_info->iBullets * recoil_index, &r4, &r3);

		else {
			r3 = c_csgo::get()->RandomFloat(0.f, 1.f);
			r4 = c_csgo::get()->RandomFloat(0.f, math::twopi);
		}
	...
	__forceinline std::string GetLocalizedName() {
		...
		item_view = c_csgo::get()->GetEconItemView(this);
		if (!item_view)
			return XOR("error");

		item_def = c_csgo::get()->GetStaticData(item_view);
		if (!item_def)
			return XOR("error");

		return util::WideToMultiByte(c_csgo::get()->m_localize->Find(item_def->GetItemBaseName()));
	}
};
...
public:
	__forceinline CTraceFilterSimple_game() :
		m_vmt{ c_csgo::get()->CTraceFilterSimple_vmt.as< void * >() },
		m_pass_ent1{},
		m_collision_group{},
...
public:
	__forceinline CTraceFilterSkipTwoEntities_game() :
		m_vmt{ c_csgo::get()->CTraceFilterSkipTwoEntities_vmt.as< void * >() },
		m_pass_ent1{},
		m_collision_group{},
		m_shouldhit_check_fn{},
		m_pass_ent2{} {}
