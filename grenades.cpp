...
void c_grenades::paint() {
	static CTraceFilterSimple_game filter{};
	CGameTrace	                   trace;
	std::pair< float, Player* >    target{ 0.f, nullptr };

	if (!c_config::get()->b["grenade_tracer"])
		return;
	...
	for (int i{ 1 }; i <= c_csgo::get()->m_globals->m_max_clients; ++i) {
		...
		if (m_id == HEGRENADE) {
			...
			c_csgo::get()->m_engine_trace->TraceRay(Ray(prev, center), MASK_SHOT, (ITraceFilter*)&filter, &trace);
...
void c_grenades::think() {
	...
	m_id = c_client::get()->m_weapon_id;
	m_power = c_client::get()->m_weapon->m_flThrowStrength();
	m_vel = c_client::get()->m_weapon_info->flThrowVelocity;

	simulate();
}

void c_grenades::simulate() {
	...
	if (m_id == MOLOTOV || m_id == FIREBOMB) {
		CGameTrace trace;
		PhysicsPushEntity(m_start, { 0.f, 0.f, -131.f }, trace, c_client::get()->m_local);
...
void c_grenades::setup() {
	// get the last CreateMove angles.
	ang_t angle = c_client::get()->m_view_angles;
	...
	m_start = c_client::get()->m_shoot_pos;
	...
	CGameTrace trace;
	TraceHull(m_start, end, trace, c_client::get()->m_local);
	...
	// weird formula, valve..
	m_velocity = c_client::get()->m_local->m_vecVelocity();
	m_velocity *= 1.25f;
	m_velocity += (forward * vel);
}

size_t c_grenades::advance(size_t tick) {
	size_t     flags{ NONE };
	CGameTrace trace;
	...
	// move object.
	PhysicsPushEntity(m_start, m_move, trace, c_client::get()->m_local);
...
bool c_grenades::detonate(size_t tick, CGameTrace& trace) {
	...
	case FIREBOMB:
		...
		if (trace.m_fraction != 1.f && (std::cos(math::deg_to_rad(c_csgo::get()->weapon_molotov_maxdetonateslope->GetFloat())) <= trace.m_plane.m_normal.z))
			return true;

		// detonate if we have traveled for too long.
		// checked every 0.1s
		return time >= c_csgo::get()->molotov_throw_detonate_time->GetFloat() && !(tick % game::TIME_TO_TICKS(0.1f));
...
void c_grenades::ResolveFlyCollisionBounce(CGameTrace& trace) {
	...
	if (trace.m_plane.m_normal.z > 0.7f) {
		...
		if (speed < 400.f)
			...
			// advance forward.
			PhysicsPushEntity(trace.m_endpos, velocity * (left * c_csgo::get()->m_globals->m_interval), trace, c_client::get()->m_local);
		}
	}
...
void c_grenades::PhysicsPushEntity(vec3_t& start, const vec3_t& move, CGameTrace& trace, Entity* ent) {
	// compute end point.
...
void c_grenades::TraceHull(const vec3_t& start, const vec3_t& end, CGameTrace& trace, Entity* ent) {
	// create trace filter.
	static CTraceFilterSimple_game filter{};

	filter.SetPassEntity(ent);

	c_csgo::get()->m_engine_trace->TraceRay(Ray(start, end, { -2.f, -2.f, -2.f }, { 2.f, 2.f, 2.f }), MASK_SOLID, (ITraceFilter*)&filter, &trace);
}

void c_grenades::PhysicsAddGravityMove(vec3_t& move) {
	...
	move.x = m_velocity.x * c_csgo::get()->m_globals->m_interval;
	move.y = m_velocity.y * c_csgo::get()->m_globals->m_interval;

	// apply linear acceleration due to gravity.
	// calculate new z velocity.
	float z = m_velocity.z - (gravity * c_csgo::get()->m_globals->m_interval);

	// apply velocity to move, the average of the new and the old.
	move.z = ((m_velocity.z + z) / 2.f) * c_csgo::get()->m_globals->m_interval;

	// write back new gravity corrected z-velocity.
	m_velocity.z = z;
}

void c_grenades::PhysicsClipVelocity(const vec3_t& in, const vec3_t& normal, vec3_t& out, float overbounce) {
