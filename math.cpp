...
void math::AngleMatrix( const ang_t& ang, const vec3_t& pos, matrix3x4_t& out ) {
    c_csgo::get()->AngleMatrix( ang, out );
    out.SetOrigin( pos );
}
...
matrix3x4_t math::AngleMatrix(const ang_t angles)
{
	...
	m128 angle, sin, cos;
	angle.f[0] = deg_to_rad(angles.x);
	angle.f[1] = deg_to_rad(angles.y);
	angle.f[2] = deg_to_rad(angles.z);
...
void math::AngleVectors_2(const vec3_t& angles, vec3_t* forward)
{
	...
	sy = std::sin(deg_to_rad(angles[1]));
	cy = std::cos(deg_to_rad(angles[1]));

	sp = std::sin(deg_to_rad(angles[0]));
	cp = std::cos(deg_to_rad(angles[0]));
...
ang_t math::vec_to_ang(const vec3_t& vecSource) {
	return ang_t(vecSource.x, vecSource.y, vecSource.z);
}

vec3_t math::ang_to_vec(const ang_t& angSource) {
	return vec3_t(angSource.x, angSource.y, angSource.z);
}
...
{
	return vec3_t(in1.dot(vec3_t(in2[0][0], in2[0][1], in2[0][2])), in1.dot(vec3_t(in2[1][0], in2[1][1], in2[1][2])), in1.dot(vec3_t(in2[2][0], in2[2][1], in2[2][2])));
}
