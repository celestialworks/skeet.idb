...
public:
	...
	__forceinline ~LagRecord() {
		...
		c_csgo::get()->m_mem_alloc->Free(m_bones);
	}
	...
	__forceinline void invalidate() {
		...
		// allocate new memory.
		m_bones = (BoneArray*)c_csgo::get()->m_mem_alloc->Alloc(sizeof(BoneArray) * 128);
	}
	...
	void store(Player* player) {
		...
		m_tick = c_csgo::get()->m_cl->clock_drift_mgr.m_server_tick;
		...
		// compute animtime.
		m_anim_time = m_old_sim_time + c_csgo::get()->m_globals->m_interval;
	}
	...
	bool valid() {
		...
		float curtime = game::TICKS_TO_TIME(c_client::get()->m_local->m_nTickBase());

		// correct is the amount of time we have to correct game time,
		float correct = c_client::get()->m_lerp + c_client::get()->m_latency;

		// stupid fake latency goes into the incoming latency.
		float in = c_csgo::get()->m_net->GetLatency(INetChannel::FLOW_INCOMING);
		correct += in;

		// check bounds [ 0, sv_maxunlag ]
		math::clamp(correct, 0.f, c_csgo::get()->sv_maxunlag->GetFloat());

		// calculate difference between tick sent by player and our latency based tick.
		// ensure this record isn't too old.
		return std::abs(correct - (curtime - m_sim_time)) <= 0.2f;

		// note: simv0l - not best way, but onetap & nemesis do it. special thanks to soufiw for this part.
		//return std::abs(correct - ((curtime - (c_client::get()->can_use_exploit ? game::TICKS_TO_TIME(c_client::get()->shift_value) : 0.f)) - m_sim_time)) <= 0.2f;
	}
};