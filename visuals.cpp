...
void c_visuals::ModulateWorld() {
	std::vector< IMaterial* > world, props;

	// iterate material handles.
	for (uint16_t h{ c_csgo::get()->m_material_system->FirstMaterial() }; h != c_csgo::get()->m_material_system->InvalidMaterial(); h = c_csgo::get()->m_material_system->NextMaterial(h)) {
		// get material from handle.
		IMaterial* mat = c_csgo::get()->m_material_system->GetMaterial(h);
		if (!mat)
			continue;
	...
	// night.
	if (c_config::get()->i["visuals_color_modulation_mode"]) {
		for (const auto& w : world)
			w->ColorModulate(0.05f, 0.05f, 0.05f);

		// IsUsingStaticPropDebugModes my nigga
		if (c_csgo::get()->r_DrawSpecificStaticProp->GetInt() != 0) {
			c_csgo::get()->r_DrawSpecificStaticProp->SetValue(0);
		}
	...
	// disable night.
		...
		if (c_csgo::get()->r_DrawSpecificStaticProp->GetInt() != -1) {
			c_csgo::get()->r_DrawSpecificStaticProp->SetValue(-1);
		}
...
void c_visuals::ThirdpersonThink() {
	ang_t                          offset;
	...
	bool alive = c_client::get()->m_local && c_client::get()->m_local->alive();

	// camera should be in thirdperson.
	if (c_key_handler::get()->auto_check("thirdperson_key")) {

		// if alive and not in thirdperson already switch to thirdperson.
		if (alive && !c_csgo::get()->m_input->CAM_IsThirdPerson())
			c_csgo::get()->m_input->CAM_ToThirdPerson();

		// if dead and spectating in firstperson switch to thirdperson.
		else if (c_client::get()->m_local->m_iObserverMode() == 4) {

			// if in thirdperson, switch to firstperson.
			// we need to disable thirdperson to spectate properly.
			if (c_csgo::get()->m_input->CAM_IsThirdPerson()) {
				c_csgo::get()->m_input->CAM_ToFirstPerson();
				c_csgo::get()->m_input->m_camera_offset.z = 0.f;
			}

			c_client::get()->m_local->m_iObserverMode() = 5;
		}
	}

	// camera should be in firstperson.
	else if (c_csgo::get()->m_input->CAM_IsThirdPerson()) {
		c_csgo::get()->m_input->CAM_ToFirstPerson();
		c_csgo::get()->m_input->m_camera_offset.z = 0.f;
	}
	...
	if (c_csgo::get()->m_input->CAM_IsThirdPerson()) {
		// get camera angles.
		c_csgo::get()->m_engine->GetViewAngles(offset);
		...
		// start pos.
		origin = c_client::get()->m_shoot_pos;

		// setup trace filter and trace.
		filter.SetPassEntity(c_client::get()->m_local);
		...
		c_csgo::get()->m_input->m_camera_offset = { offset.x, offset.y, offset.z };
	}
}
...
void c_visuals::Hitmarker() {
	...
	if (c_csgo::get()->m_globals->m_curtime > m_hit_end)
		return;

	if (m_hit_duration <= 0.f)
		return;

	float complete = (c_csgo::get()->m_globals->m_curtime - m_hit_start) / m_hit_duration;
	int x = c_client::get()->m_width / 2,
		y = c_client::get()->m_height / 2,
...
void c_visuals::NoSmoke() {
	if (!smoke1)
		smoke1 = c_csgo::get()->m_material_system->FindMaterial(XOR("particle/vistasmokev1/vistasmokev1_fire"), XOR("Other textures"));

	if (!smoke2)
		smoke2 = c_csgo::get()->m_material_system->FindMaterial(XOR("particle/vistasmokev1/vistasmokev1_smokegrenade"), XOR("Other textures"));

	if (!smoke3)
		smoke3 = c_csgo::get()->m_material_system->FindMaterial(XOR("particle/vistasmokev1/vistasmokev1_emods"), XOR("Other textures"));

	if (!smoke4)
		smoke4 = c_csgo::get()->m_material_system->FindMaterial(XOR("particle/vistasmokev1/vistasmokev1_emods_impactdust"), XOR("Other textures"));
	...
	if (c_config::get()->b["no_smoke"]) {
		...
		if (c_csgo::get()->SmokeCount.get() != 0)
			c_csgo::get()->SmokeCount.set(0);
	}
...
void c_visuals::think() {
	...
	if (c_config::get()->b["visuals_noscope"]
		&& c_client::get()->m_local->alive()
		&& c_client::get()->m_local->GetActiveWeapon()
		&& c_client::get()->m_local->m_bIsScoped()) {

		int w, h = 0;
		c_csgo::get()->m_engine->GetScreenSize(w, h);
		int gOffy = h / 2 / 2;
		render::gradient_horizontal(w / 2 - gOffy, h / 2, gOffy - 8, 1, Color(c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).r(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).g(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).b(), 0), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]));
		render::gradient_horizontal(w / 2 + 8, h / 2, gOffy, 1, c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]), Color(c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).r(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).g(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).b(), 0));
		render::gradient_vertical(w / 2, h / 2 - gOffy, 1, gOffy - 8, Color(c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).r(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).g(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).b(), 0), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]));
		render::gradient_vertical(w / 2, h / 2 + 8, 1, gOffy, c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]), Color(c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).r(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).g(), c_config::get()->imcolor_to_ccolor(c_config::get()->c["noscope_color"]).b(), 0));
	}

	// draw esp on ents.
	for (int i{ 1 }; i <= c_csgo::get()->m_entlist->GetHighestEntityIndex(); ++i) {
		Entity* ent = c_csgo::get()->m_entlist->GetClientEntity(i);
		if (!ent)
			continue;
...
void c_visuals::Spectators() {
	...
	for (int i{ 1 }; i <= c_csgo::get()->m_globals->m_max_clients; ++i) {
		...
		if (player->GetObserverTarget() != c_client::get()->m_local)
			continue;

		player_info_t info;
		if (!c_csgo::get()->m_engine->GetPlayerInfo(i, &info))
			continue;
	...
	for (size_t i{ }; i < spectators.size(); ++i) {
		...
		render::menu_shade.string(c_client::get()->m_width - 20, (c_client::get()->m_height / 2) - (total_size / 2) + (i * (h - 1)),
			{ 255, 255, 255, 179 }, name, render::ALIGN_RIGHT);
	}
}

void c_visuals::keybinds() {
	...
	// cheat variables.
	std::string logo = "keybinds";

	render::FontSize_t size = render::hud.size(logo);

	// background.
	render::rect_filled(c_client::get()->m_width - size.m_width - 45, 400, size.m_width + 8, size.m_height + 8, col_background);
	render::rect_filled(c_client::get()->m_width - size.m_width - 45, 400, size.m_width + 8, 2, col_accent);

	// text.
	render::hud.string(c_client::get()->m_width - 41, 404, col_text, logo, render::ALIGN_RIGHT);

	std::vector<std::string> binds;
	
	if (c_key_handler::get()->auto_check("rbot_baim_key"))
		binds.push_back("baim");

	if (c_key_handler::get()->auto_check("exploits_key"))
		binds.push_back("exploits");

	if (c_key_handler::get()->auto_check("desync_invert_key"))
		binds.push_back("desync inverter");

	if (c_key_handler::get()->auto_check("fakeduck_key"))
		binds.push_back("duck peek assist");

	int padding = 0;
	for (auto key : binds) {
		render::hud.string(c_client::get()->m_width - size.m_width - 45, 420 + padding, col_text, key);
		padding += 10;
	}
}

void c_visuals::StatusIndicators() {
	// dont do if dead.
	if (!c_client::get()->m_processing)
		return;
	
	// compute hud size.
	// int size = ( int )std::round( ( c_client::get()->m_height / 17.5f ) * c_csgo::get()->hud_scaling->GetFloat( ) );

	struct Indicator_t { Color color; std::string text; };
	std::vector< Indicator_t > indicators{ };

	// LC
	if (c_client::get()->m_local->m_vecVelocity().length_2d() > 270.f || c_client::get()->m_lagcomp) {
		Indicator_t ind{ };
		ind.color = c_client::get()->m_lagcomp ? 0xff15c27b : 0xff0000ff;
		ind.text = XOR("LC");
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string(10, c_client::get()->m_height - 80 - (30 * i), indicator.color, indicator.text);
	}
}

void c_visuals::SpreadCrosshair() {
	// dont do if dead.
	if (!c_client::get()->m_processing)
		return;

	if (!c_config::get()->b["visuals_spread_crosshair"])
		return;

	// get active weapon.
	Weapon* weapon = c_client::get()->m_local->GetActiveWeapon();
	if (!weapon)
		return;
	...
	radius *= c_client::get()->m_height * (1.f / 480.f);

	// get color.
	Color col = c_config::get()->imcolor_to_ccolor(c_config::get()->c["spread_crosshair_color"]);

	int segements = std::max(16, (int)std::round(radius * 0.75f));
	render::circle(c_client::get()->m_width / 2, c_client::get()->m_height / 2, radius, segements, col);
}

void c_visuals::PenetrationCrosshair() {
	int   x, y;
	...
	x = c_client::get()->m_width / 2;
	y = c_client::get()->m_height / 2;


	valid_player_hit = (c_client::get()->m_pen_data.m_target && c_client::get()->m_pen_data.m_target->enemy(c_client::get()->m_local));
	if (valid_player_hit)
		final_color = colors::transparent_yellow;

	else if (c_client::get()->m_pen_data.m_pen)
...
void c_visuals::draw(Entity* ent) {
	if (ent->IsPlayer()) {
	...
	else if (c_config::get()->b["visuals_items"] && ent->IsBaseCombatWeapon() && !ent->dormant())
		DrawItem(ent->as< Weapon* >());

	else if (c_config::get()->b["visuals_projecties"])
		DrawProjectile(ent->as< Weapon* >());
}

void c_visuals::DrawProjectile(Weapon* ent) {
	vec2_t screen;
	vec3_t origin = ent->GetAbsOrigin();
	if (!render::WorldToScreen(origin, screen))
		return;

	Color col = c_config::get()->imcolor_to_ccolor(c_config::get()->c["projecties_color"]);
	col.a() = 0xb4;
...
void c_visuals::DrawItem(Weapon* item) {
	// we only want to draw shit without owner.
	Entity* owner = c_csgo::get()->m_entlist->GetClientEntityFromHandle(item->m_hOwnerEntity());
	if (owner)
		return;
	...
	if (!c_config::get()->b["visuals_ammo"])
		return;
...
void c_visuals::OffScreen(Player* player, int alpha) {
	...
	static auto get_offscreen_data = [](const vec3_t& delta, float radius, vec2_t& out_offscreen_pos, float& out_rotation) {
		ang_t  view_angles(c_csgo::get()->m_view_render->m_view.m_angles);
		...
		out_offscreen_pos.x = (int)((c_client::get()->m_width / 2.f) + (radius * sa));
		out_offscreen_pos.y = (int)((c_client::get()->m_height / 2.f) - (radius * ca));
	};

	if (!c_config::get()->b["visuals_offscreen"])
		return;

	if (!c_client::get()->m_processing || !c_client::get()->m_local->enemy(player))
		return;
	...
	leeway_x = c_client::get()->m_width / 18.f;
	leeway_y = c_client::get()->m_height / 18.f;

	// origin is not on the screen at all, get offscreen position data and start rendering.
	if (!is_on_screen
		|| screen_pos.x < -leeway_x
		|| screen_pos.x >(c_client::get()->m_width + leeway_x)
		|| screen_pos.y < -leeway_y
		|| screen_pos.y >(c_client::get()->m_height + leeway_y)) {

		// get viewport origin.
		view_origin = c_csgo::get()->m_view_render->m_view.m_origin;

		// get direction to target.
		delta = (target_pos - view_origin).normalized();

		// note - dex; this is the 'YRES' macro from the source sdk.
		radius = 200.f * (c_client::get()->m_height / 480.f);
		...
		//     //     // float step = UINT8_MAX / ( 1000.f * c_csgo::get()->m_globals->m_frametime );
		//     //     float step = ( 1.f / c_csgo::get()->m_globals->m_frametime ) / UINT8_MAX;
		//     //     
		...
		// render!
		color = c_config::get()->imcolor_to_ccolor(c_config::get()->c["offscreen_color"]); // damage_data.m_color;
		color.a() = (alpha == 255) ? alpha : alpha / 2;

		c_csgo::get()->m_surface->DrawSetColor(color);
		c_csgo::get()->m_surface->DrawTexturedPolygon(3, verts);

		// c_csgo::get()->m_surface->DrawSetColor( colors::black );
		// c_csgo::get()->m_surface->DrawTexturedPolyLine( 3, verts_outline );
	}
}
...
bool IsFakeDucking(Player* player) {
	...
	if (storedTick != c_csgo::get()->m_globals->m_tick_count)
	{
		crouchedTicks[player->index()]++;
		storedTick = c_csgo::get()->m_globals->m_tick_count;
	}
...
void c_visuals::DrawPlayer(Player* player) {
	...
	float step = frequency * c_csgo::get()->m_globals->m_frametime;

	// is player enemy.
	bool enemy = player->enemy(c_client::get()->m_local);
	...
	float dt = c_csgo::get()->m_globals->m_curtime - player->m_flSimulationTime();
	...
	color = c_config::get()->imcolor_to_ccolor(c_config::get()->c["box_color"]);
	...
	bool bone_esp = (enemy && c_config::get()->b["visuals_skeleton"]);
	if (bone_esp)
		DrawSkeleton(player, alpha);

	// is box esp enabled for this player.
	bool box_esp = (enemy && c_config::get()->b["visuals_box"]);
	...
	bool name_esp = (enemy && c_config::get()->b["visuals_name"]);
	...
	if (name_esp) {
		...
		Color clr = c_config::get()->imcolor_to_ccolor(c_config::get()->c["box_color"]);
		// override alpha.
		clr.a() = low_alpha;
	...
	bool health_esp = (enemy && c_config::get()->b["visuals_health"]);
	...
	// draw bottom bars.
	{
		...
		// draw weapon.
		if ((enemy && c_config::get()->b["visuals_weapon"])) {
			...
			if (weapon) {
				...
				if (data) {
					...
					// ammo bar.
					if (max != -1 && c_config::get()->b["visuals_ammo"]) {
						// check for reload.
						if (reload)
...
void c_visuals::DrawPlantedC4() {
	...
	//     https://github.com/ValveSoftware/source-sdk-2013/blob/master/sp/src/game/shared/sdk/sdk_gamerules.cpp#L173
	{
		...
		dst = c_client::get()->m_local->WorldSpaceCenter();
		...
		final_damage = scale_damage(range_damage, c_client::get()->m_local->m_ArmorValue());
	}

	// m_flC4Blow is set to gpGlobals->curtime + m_flTimerLength inside CPlantedC4.
	explode_time_diff = m_planted_c4_explode_time - c_csgo::get()->m_globals->m_curtime;
	...
	// get damage color.
	damage_color = (final_damage < c_client::get()->m_local->m_iHealth()) ? colors::white : colors::red;
	...
	// 'on screen (2D)'.
	if (mode_2d) {
		// c_client::get()->m_height - 80 - ( 30 * i )
		// 80 - ( 30 * 2 ) = 20

		// render::menu_shade.string( 60, c_client::get()->m_height - 20 - ( render::hud_size.m_height / 2 ), 0xff0000ff, "", render::hud );
...
bool c_visuals::GetPlayerBoxRect(Player* player, Rect& box) {
...
void c_visuals::DrawHistorySkeleton(Player* player, int opacity) {
	const model_t* model;
	studiohdr_t* hdr;
	mstudiobone_t* bone;
	AimPlayer* data;
	...
	// get studio model.
	hdr = c_csgo::get()->m_model_info->GetStudioModel(model);
	if (!hdr)
		return;

	data = &c_aimbot::get()->m_players[player->index() - 1];
	if (!data)
		return;
...
void c_visuals::DrawSkeleton(Player* player, int opacity) {
	const model_t* model;
	studiohdr_t* hdr;
	mstudiobone_t* bone;
	int           parent;
	...
	// get bone matrix.
	if (!player->SetupBones(matrix, 128, BONE_USED_BY_ANYTHING, c_csgo::get()->m_globals->m_curtime))
		return;
...
void c_visuals::RenderGlow() {
	...
	if (!c_csgo::get()->m_glow->m_object_definitions.Count())
		return;

	float blend = c_config::get()->i["glow_blend"] / 100.f;

	for (int i{ }; i < c_csgo::get()->m_glow->m_object_definitions.Count(); ++i) {
		GlowObjectDefinition_t* obj = &c_csgo::get()->m_glow->m_object_definitions[i];
		...
		if (enemy && !c_config::get()->b["visuals_glow"])
			continue;

		// enemy color
		if (enemy)
			color = c_config::get()->imcolor_to_ccolor(c_config::get()->c["glow_color"]);
...
void c_visuals::DrawHitboxMatrix(LagRecord* record, Color col, float time) {
	const model_t* model;
	studiohdr_t* hdr;
	...
	for (int i{ }; i < set->m_hitboxes; ++i) {
		...
		if (bbox->m_radius <= 0.f) {
			...
			// draw box.
			c_csgo::get()->m_debug_overlay->AddBoxOverlay(origin, bbox->m_mins, bbox->m_maxs, bbox_angle, col.r(), col.g(), col.b(), 0, time);
		}
		...
		// capsule.
			...
			matrix3x4_t matrix;
			c_csgo::get()->AngleMatrix(bbox->m_angle, matrix);
			...
			c_csgo::get()->m_debug_overlay->AddCapsuleOverlay(mins, maxs, bbox->m_radius, col.r(), col.g(), col.b(), col.a(), time, 0, 0);
		}
	}
}

void c_visuals::DrawBeams() {
	size_t     impact_count;
	float      curtime, dist;
	...
	auto vis_impacts = &c_shots::get()->m_vis_impacts;
	...
	if (!c_client::get()->m_processing) {
		...
		curtime = game::TICKS_TO_TIME(c_client::get()->m_local->m_nTickBase());
		...
		for (size_t i{ impact_count }; i-- > 0; ) {
			...
			if (std::abs(curtime - game::TICKS_TO_TIME(impact->m_tickbase)) > 2) {
			...
			if (is_final_impact) {
				...
				beam_info.m_nModelIndex = c_csgo::get()->m_model_info->GetModelIndex(XOR("sprites/physbeam.vmt"));
				beam_info.m_pszModelName = XOR("sprites/physbeam.vmt");
				beam_info.m_flHaloScale = 0.f;
				beam_info.m_flLife = 2;
				beam_info.m_flWidth = 2.0f;
				beam_info.m_flEndWidth = 2.0f;
				beam_info.m_flFadeLength = 0.f;
				beam_info.m_flAmplitude = 3.f;   // beam 'jitter'.
				...
				beam_info.m_flRed = c_config::get()->imcolor_to_ccolor(c_config::get()->c["beams_color"]).r();
				beam_info.m_flGreen = c_config::get()->imcolor_to_ccolor(c_config::get()->c["beams_color"]).g();
				beam_info.m_flBlue = c_config::get()->imcolor_to_ccolor(c_config::get()->c["beams_color"]).b();

				// attempt to render the beam.

				beam = c_csgo::get()->m_beams->CreateBeamPoints(beam_info);
				if (beam) {
					c_csgo::get()->m_beams->DrawBeam(beam);
