...
class c_csgo : public singleton<c_csgo> {
public:
...
namespace game {
	...
	{
		static DWORD* m_pThis = *pattern::find(c_csgo::get()->m_client_dll, XOR("B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08")).add(0x1).as< DWORD**>();
	...
	__forceinline float GetClientInterpAmount() {
		return std::max(c_csgo::get()->cl_interp->GetFloat(), c_csgo::get()->cl_interp_ratio->GetFloat() / c_csgo::get()->cl_updaterate->GetFloat());
	}

	__forceinline int TIME_TO_TICKS(float time) {
		return (int)(0.5f + time / c_csgo::get()->m_globals->m_interval);
	}

	__forceinline float TICKS_TO_TIME(int ticks) {
		return c_csgo::get()->m_globals->m_interval * (float)(ticks);
	}
	...
	inline void CreateAnimationState(Player *holder, CCSGOPlayerAnimState *state) {
		...
		static auto func = pattern::find(c_csgo::get()->m_client_dll, XOR("55 8B EC 56 8B F1 B9 ? ? ? ? C7 46")).as< CreateAnimState_t >();
		if (!func)
			return;
	...
	inline void UpdateAnimationState(CCSGOPlayerAnimState* state, ang_t ang) {
		...
		static auto ret = pattern::find(c_csgo::get()->m_client_dll, XOR("55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24")).as< fn >();
	...
	inline void ResetAnimationState(CCSGOPlayerAnimState* state) {
		...
		static auto func = pattern::find(c_csgo::get()->m_client_dll, XOR("56 6A 01 68 ? ? ? ? 8B F1")).as< ResetAnimState_t >();
		if (!func)
			return;
	...
	__forceinline void UpdateAllViewmodelAddons(Entity* entity) {
		...
		static UpdateAllViewmodelAddons_t UpdateAllViewmodelAddonsFn = pattern::find(c_csgo::get()->m_client_dll, XOR("55 8B EC 83 E4 ? 83 EC ? 53 8B D9 56 57 8B 03 FF 90 ? ? ? ? 8B F8 89 7C 24 ? 85 FF 0F 84 ? ? ? ? 8B 17 8B CF")).as<UpdateAllViewmodelAddons_t>();
	...
	__forceinline void SetSkybox(const char* name) {
		...
		static auto load_named_sky = pattern::find(c_csgo::get()->m_engine_dll, XOR("55 8B EC 81 EC ? ? ? ? 56 57 8B F9 C7 45")).as< void(__fastcall*)(const char*) >();
