...
Weapon* Hooks::GetActiveWeapon() {
	...
	static Address ret_1 = pattern::find(c_csgo::get()->m_client_dll, XOR("85 C0 ? ? 8B 88 ? ? ? ? 85 C9 ? ? 80 39 00 ? ? 51 56"));
	...
	if (ent) {
		...
		if (ent->IsPlayer()) {
			...
			if (vmt) {
				...
				if (player->index() == c_csgo::get()->m_engine->GetLocalPlayer()) {
		...
		// 
		//     c_client::get()->print( "ragdoll 0x%p created on client at time %f, from player 0x%p\n", ent, c_csgo::get()->m_globals->m_curtime, ragdoll_player );
		// }
	}
}
