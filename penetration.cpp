...
bool penetration::HandleBulletPenetration(WeaponInfo* info, FireBulletData_t& data, bool extracheck, vec3_t point)
{
	...
	surfacedata_t* enter_surface_data = c_csgo::get()->m_phys_props->GetSurfaceData(data.m_enter_trace.m_surface.m_surface_props);
	int enter_material = enter_surface_data->m_game.m_material;

	static auto dbp = c_csgo::get()->m_cvar->FindVar(HASH("ff_damage_bullet_penetration"));
	...
	if (data.m_penetration_count <= 0
		...
		&& !(c_csgo::get()->m_engine_trace->GetPointContents(data.m_enter_trace.m_endpos, MASK_SHOT_HULL | CONTENTS_HITBOX, NULL) & (MASK_SHOT_HULL | CONTENTS_HITBOX)))
		return false;

	surfacedata_t* exit_surface_data = c_csgo::get()->m_phys_props->GetSurfaceData(trace_exit.m_surface.m_surface_props);
	...
	if (extracheck) {
		static auto VectortoVectorVisible = [&](vec3_t src, vec3_t point) -> bool {
			...
			TraceLine(src, point, MASK_SOLID, c_client::get()->m_local, &TraceInit);
			CGameTrace Trace;
...
void penetration::TraceLine(vec3_t& start, vec3_t& end, unsigned int mask, Entity* ignore, CGameTrace* trace)
{
	...
	c_csgo::get()->m_engine_trace->TraceRay(Ray(start, end), mask, &filter, trace);
}
...
bool penetration::TraceToExit(const vec3_t &start, const vec3_t& dir, vec3_t& out, CGameTrace* enter_trace, CGameTrace* exit_trace) {
	...
	while (dist <= 90.f) {
		...
		if (!first_contents)
			first_contents = c_csgo::get()->m_engine_trace->GetPointContents(out, MASK_SHOT, nullptr);

		contents = c_csgo::get()->m_engine_trace->GetPointContents(out, MASK_SHOT, nullptr);
		...
		// do first trace.
		c_csgo::get()->m_engine_trace->TraceRay(Ray(out, new_end), MASK_SHOT, nullptr, exit_trace);

		// note - dex; this is some new stuff added sometime around late 2017 ( 10.31.2017 update? ).
		if (c_csgo::get()->sv_clip_penetration_traces_to_players->GetInt())
		...
		if (exit_trace->m_startsolid && (exit_trace->m_surface.m_flags & SURF_HITBOX)) {
			...
			c_csgo::get()->m_engine_trace->TraceRay(Ray(out, start), MASK_SHOT_HULL, (ITraceFilter *)&filter, exit_trace);
...
void penetration::ClipTraceToPlayer(const vec3_t& start, const vec3_t& end, uint32_t mask, CGameTrace* tr, Player* player, float min) {
	...
	if ( /*min <= range &&*/ range <= 60.f) {
		// clip to player.
		c_csgo::get()->m_engine_trace->ClipRayToEntity(Ray(start, end), mask, player, &new_trace);
...
bool penetration::CanHitFloatingPoint(const vec3_t& point, const vec3_t& source) {
	...
	data.m_filter->skip_entity = c_client::get()->m_local;
	...
	auto weaponData = c_client::get()->m_local->GetActiveWeapon()->GetWpnData();
	...
	TraceLine(data.m_start, end, MASK_SHOT | CONTENTS_HITBOX, c_client::get()->m_local, &data.m_enter_trace);
...
bool penetration::run(PenetrationInput_t* in, PenetrationOutput_t* out) {
	...
	if (in->m_from == c_client::get()->m_local) {
		weapon = c_client::get()->m_weapon;
		weapon_info = c_client::get()->m_weapon_info;
		start = c_client::get()->m_shoot_pos;
	}
	...
	while (damage > 0.f) {
		...
		c_csgo::get()->m_engine_trace->TraceRay(Ray(start, end), MASK_SHOT | CONTENTS_HITBOX, (ITraceFilter *)&filter, &trace);
		...
		enter_surface = c_csgo::get()->m_phys_props->GetSurfaceData(trace.m_surface.m_surface_props);
		...
		if (!TraceToExit(trace.m_endpos, dir, pen_end, &trace, &exit_trace)) {
			if (!(c_csgo::get()->m_engine_trace->GetPointContents(pen_end, MASK_SHOT_HULL) & MASK_SHOT_HULL))
				return false;
		}

		// get surface / material at exit point.
		exit_surface = c_csgo::get()->m_phys_props->GetSurfaceData(exit_trace.m_surface.m_surface_props);
