...
public:
    ...
    void init( ) {
		...
		if( !c_csgo::get()->m_client )
			return;

		// grab linked list.
		list = c_csgo::get()->m_client->GetAllClasses( );
		if( !list )
			return;
	...
	void FindAndStoreDataMaps( ) {
		...
		// sanity.
		if( !c_csgo::get()->m_client_dll )
			return;

		matches = pattern::FindAll( c_csgo::get()->m_client_dll, XOR( "C7 05 ? ? ? ? ? ? ? ? C7 05 ? ? ? ? ? ? ? ? C3 CC" ) );
		if( matches.empty( ) )
			return;
