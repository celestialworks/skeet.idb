...
namespace render {
	...
	public:
		...
		__forceinline Font( const std::string& name, int s, int w, int flags ) {
			m_handle = c_csgo::get()->m_surface->CreateFont( );
			c_csgo::get()->m_surface->SetFontGlyphSet( m_handle, name.data( ), s, w, 0, 0, flags );
			m_size = size( XOR( "A" ) );
		}
	...
	void gradient_vertical( int x, int y, int w, int h, Color color1, Color color2 );
	void gradient_horizontal( int x, int y, int w, int h, Color color1, Color color2 );
