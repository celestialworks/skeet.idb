...
void GameMovement::Extrapolate( Player* pEntity, vec3_t& vecOrigin, vec3_t& vecVelocity, int& fFlags, bool bOnGround ) {
	...
	end = start + ( vecVelocity * c_csgo::get()->m_globals->m_interval );

	// trace.
	c_csgo::get()->m_engine_trace->TraceRay( Ray( start, end, pEntity->m_vecMins(), pEntity->m_vecMaxs() ), CONTENTS_SOLID, &filter, &trace );
	...
	if( trace.m_fraction != 1.f ) {
		...
		for( int i{ }; i < 2; ++i ) {
			...
			end = start + ( vecVelocity * ( c_csgo::get()->m_globals->m_interval * ( 1.f - trace.m_fraction ) ) );
