...
bool c_csgo::init( ) {
...
#ifdef _DEBUG
	Address debugbreak = pattern::find( c_csgo::get()->m_client_dll, XOR( "CC F3 0F 10 4D ? 0F 57 C0" ) );
	...
	std::filesystem::create_directory(XOR("scripts\\"));
	...
	if( m_engine->IsInGame( ) ) {
		...
		c_csgo::get()->cl_fullupdate->m_callback( );
	}
...
bool game::IsBreakable(Entity* ent) {
	...
	static size_t m_takedamage_offset{ *(size_t*)((uintptr_t)c_csgo::get()->IsBreakableEntity + 0x26) };
	...
	ret = c_csgo::get()->IsBreakableEntity(ent);
	*takedmg = old_takedmg;
...
void game::UTIL_ClipTraceToPlayers(const vec3_t& start, const vec3_t& end, uint32_t mask, ITraceFilter* filter, CGameTrace* tr, float range) {
	static auto func = pattern::find(c_csgo::get()->m_client_dll, XOR("E8 ? ? ? ? 83 C4 14 8A 56 37")).rel32< uintptr_t >(0x1);
	if (!func)
		return;
